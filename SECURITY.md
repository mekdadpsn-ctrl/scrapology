...
- Scrapology executes no code from fetched pages outside the browser sandbox Playwright provides,
  ...
  preserved by refusing what they would have stopped: every cross-origin request in CORS mode
  (fetch, XHR, EventSource, module scripts, fonts, preloads, `crossorigin` resources, recognised by
  the Origin header the browser sends), every CORS-mode request from an opaque origin such as a
  sandboxed frame, non-GET requests made by pages, and requests from public pages to private,
  loopback, link-local or unspecified addresses, including IPv6 forms that embed an IPv4 address
  (checked before any connection, including robots.txt, is made). XML and HTML are parsed with
  entity expansion, DTD loading and network access switched off.
...
- Responses are capped at 50 MB by default and redirects at 5 hops, http and https only; every
  ...
  connection is not caught. The CORS refusal is stricter than a browser: a server that would have
  opted in with Access-Control-Allow-Origin is still refused, because the request's credentials mode
  is not visible to the interceptor.
